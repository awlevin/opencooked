...
// Thumb travel. Kept in step with .stick / .stick__thumb in controller.css so
// the thumb tops out flush with the base ring.
...
export class GamepadView {
  ...
  private queueMove(v: Vec2, force: boolean): void {
    ...
    if (
      !force &&
      Math.abs(v.x - this.lastSent.x) < EPSILON &&
      Math.abs(v.y - this.lastSent.y) < EPSILON
    ) {
      return;
    }
    if (force) {
